...
export default function VerifyPage() {
  ...
  useEffect(() => {
    if (error) {
      ...
      // This page is also reachable directly — redirect to API if token in URL
      // (rare case; normally the API redirects here with error params)
      setStatus('success')
    }
  }, [error])
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#08080F] relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[500px] h-[500px] rounded-full blur-[120px] opacity-10"
          style={{ background: status === 'error' ? '#ef4444' : '#7c3aed' }} />
      </div>
